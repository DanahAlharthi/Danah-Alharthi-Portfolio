...
export default function Navbar() {
  const [activeNav, setActiveNav] = useState(''); 
  // إضافة حالة لمعرفة إذا الشاشة جوال
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (window.scrollY < 300) setActiveNav('');
    };
  ...
  // --- الستايلات الديناميكية (تتغير إذا جوال) ---
  const dynamicNavBody = {
    ...navBody,
    padding: isMobile ? '5px 10px' : '8px 15px',
    gap: isMobile ? '2px' : '10px'
  };

  const dynamicNavItem = (isActive) => ({
    ...(isActive ? activeLinkStyle : navItemBase),
  ...
  const dynamicLogo = {
    ...logoStyle,
    display: isMobile ? 'none' : 'flex', // نخفي اللوجو في الجوال عشان نكسب مساحة للروابط
  };

  return (
    <nav style={fixedWrapper}>
      <div className="glass-nav" style={dynamicNavBody}>
        {/* Logo */}
        <div style={dynamicLogo}>DA</div>

        {/* Links Container */}
        <div style={{...linksContainer, width: isMobile ? '100%' : 'auto', justifyContent: 'space-around'}}>
          <a href="#skills" onClick={() => setActiveNav('#skills')} style={dynamicNavItem(activeNav === '#skills')}>
            Skills
          </a>
          <a href="#achievements" onClick={() => setActiveNav('#achievements')} style={dynamicNavItem(activeNav === '#achievements')}>
            Achievements
          </a>
          <a href="#projects" onClick={() => setActiveNav('#projects')} style={dynamicNavItem(activeNav === '#projects')}>
            Projects
          </a>
          <a href="#contact" onClick={() => setActiveNav('#contact')} style={dynamicNavItem(activeNav === '#contact')}>
            Contact
          </a>
        </div>
      </div>
