...
export default function Contact() {
  ...
  return (
    <section id="contact" style={{ padding: isMobile ? '20px 10px' : '30px 20px', maxWidth: '900px', margin: '0 auto' }}>
      <div style={{ 
        background: 'rgba(255, 255, 255, 0.6)', 
        backdropFilter: 'blur(20px)', 
        WebkitBackdropFilter: 'blur(20px)',
        borderRadius: isMobile ? '30px' : '40px', 
        padding: isMobile ? '25px 20px' : '40px 40px', 
        boxShadow: '0 25px 50px rgba(57, 32, 95, 0.1)',
        border: '1px solid rgba(255, 255, 255, 0.4)',
      }}>
        
        <h2 style={{ color: '#39205f', fontSize: isMobile ? '1.8rem' : '2.2rem', marginTop: '0', marginBottom: '10px', fontWeight: '800' }}>
          Email me!
        </h2>
        <p style={{ color: '#64748b', fontSize: isMobile ? '0.95rem' : '1.1rem', marginBottom: '20px' }}>
          Feel free to reach out for collaborations or just a friendly hello!
        </p>

        <form ref={form} onSubmit={sendEmail} style={{ 
          display: 'grid', 
          // السحر هنا: لو جوال خليه عمود واحد 1fr، لو لابتوب خليه عمودين 1fr 1fr
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', gridColumn: '1 / -1' }}>
            <label style={labelStyle}>Your Message</label>
            <textarea name="message" placeholder="Tell me about your project..." rows={isMobile ? "4" : "6"} required style={inputStyle}></textarea>
          </div>
          ...
          <div style={{ gridColumn: '1 / -1', marginTop: '10px' }}>
            <button type="submit" className="send-btn" style={{
              ...
              width: isMobile ? '100%' : 'auto', // الزر ياخذ العرض كامل في الجوال
              borderRadius: '18px',
              border: 'none',
              cursor: 'pointer',
              fontSize: '18px',
          ...
          {status && (
            <p style={{ gridColumn: '1 / -1', color: '#39205f', fontWeight: '600', marginTop: '15px', fontSize: '0.9rem' }}>
              {status}
            </p>
          )}
        </form>
        ...
        {/* روابط التواصل الاجتماعي */}
        <div className="social-links-container" style={{ marginTop: '20px' }}>
          <a href="http://linkedin.com/in/danahalharthi" target="_blank" rel="noreferrer" className="social-icon-wrapper">
            <FaLinkedinIn size={22} />
          </a>
          <a href="https://x.com/it_danah" target="_blank" rel="noreferrer" className="social-icon-wrapper">
            <FaXTwitter size={22} />
          </a>
        </div>
      </div>
...
const inputStyle = {
  ...
  fontFamily: 'inherit',
  width: '100%',
  boxSizing: 'border-box' // يضمن أن الـ Padding ما يطلّع الحقل برا
};